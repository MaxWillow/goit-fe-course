...
// выполняют следующие операции над массивом объектов пользователей users
...
// 3. олучить массив имен пользователей по полу (поле gender).
...
const getNamesSortedByFriendsCount = users =>
  [...users]
    .sort((a, b) => a.friends.length - b.friends.length) // по возрастанию
    .map(user => user.name);
...
const getSortedUniqueSkills = users =>
  [
    ...users
      ...
      .filter((user, idx, self) => self.indexOf(user) === idx),
  ].sort();
